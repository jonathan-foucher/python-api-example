...
def get_players_by_firstname(firstname: str, db: Session = Depends(get_db)):
    return crud.get_players_by_firstname(db, firstname=firstname)
...
def create_score_for_player(player_id: int, score: schemas.ScoreCreate, db: Session = Depends(get_db)):
    db_score = crud.get_score_by_player_id_and_game_title(db, player_id=player_id, game_title=score.game_title)
    if db_score:
